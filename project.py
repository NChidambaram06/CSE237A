...
def body_scroll(sv):
    if sv.pick == 1:
        curr_time = sv.get_millis()
        if read_gpio_value(PIN_SCROLL) and (curr_time - sv.scrollLastClick) > 100:
            print("scroll click")
            sv.scroll = 1
            sv.scrollLastClick = sv.get_millis()
            mouse.scroll(0, sv.scroll_speed*sv.scroll_dir)
        elif (curr_time - sv.scrollLastClick) > 400:
            if sv.scroll == 1:
...
def body_ultra(sv):
    ...
    if (currTime - sv.lastUltraTime) > 400 and sv.scroll == 0 and sv.pick == 1:
...
def body_tilt(sv):
    if sv.pick == 1 and sv.scroll == 1:
        curr_time = sv.get_millis()
        if (curr_time - sv.scrollLastClick) > 400:
            sv.scroll = 0
        else:
